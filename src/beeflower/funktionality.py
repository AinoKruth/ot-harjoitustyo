...
class BeeFlower:
    ...
    def __init__(self):
        ...
        self.width = 640
        self.x = 0
        self.y = self.height-self.Creativity.mehilainen.get_height()
...
#starting the game
    ...
    def new_game(self):
        self.x = 0
        self.y = 480-self.Creativity.mehilainen.get_height()
        self.amounth = 5
        self.amounth1 = 5
	...
	#pidetään hahmo sisällä
        ...
        if self.go == True:
            ...
            for i in range(self.amounth):
                ...
                if self.places[i][1]+self.Creativity.kukka.get_height() >= self.height:
                    self.places[i][0] = randint(0,self.width-self.Creativity.kukka.get_width())
                    self.places[i][1] = -randint(100,1000)

                #Defining what happens when the bee gets the flower:
                if self.places[i][1]+self.Creativity.kukka.get_height()-25 >= self.y:
                    if self.y+self.Creativity.mehilainen.get_height()-25 >= self.places[i][1]:
                        mehilainen_middle = self.x+self.Creativity.mehilainen.get_width()/2
                        kukka_middle = self.places[i][0]+self.Creativity.kukka.get_width()/2
            ...
            for i in range(self.amounth1):
                ...
                if self.places1[i][1]+self.Creativity.pisara.get_height() >= self.height:
                    self.places1[i][0] = randint(0,self.width-self.Creativity.pisara.get_width())
                    self.places1[i][1] = -randint(100,1000)

                #Defining the game to end when the bee hits a waterdrop:
                if self.places1[i][1]+self.Creativity.pisara.get_height()-25 >= self.y:
                    if self.y+self.Creativity.mehilainen.get_height()-25  >= self.places1[i][1]:
                        mehilainen_middle = self.x+self.Creativity.mehilainen.get_width()/2
                        pisara_keski = self.places1[i][0]+self.Creativity.pisara.get_width()/2
